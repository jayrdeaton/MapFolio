...
function startCapture(
  ...
  const handleMove = (e: PointerEvent) => { if (e.pointerId === pointerId) onMove(e) }
...
function buildHandles() {
  ...
  rectEl.addEventListener('pointerdown', (e: PointerEvent) => {
    e.preventDefault()
    e.stopPropagation()
    onBodyDown(e)
  }, { passive: false })
  rectEl.addEventListener('wheel', (e: WheelEvent) => {
  ...
  rotateEl.addEventListener('pointerdown', (e: PointerEvent) => {
    e.preventDefault()
    e.stopPropagation()
...
function destroyHandles() {
  handleLayer?.remove()
  handleLayer = null; rectEl = null; rotateLineEl = null; cornerEls = []; rotateEl = null; cornerIndicatorEl = null
}

function updateCornerIndicator() {
  if (!cornerIndicatorEl) return
  const c = props.overlayCorner
  if (c === undefined) { cornerIndicatorEl.style.display = 'none'; return }
...
function applyRect(state: RectState) {
  ...
  localCorners = cornerOffsets(halfW, halfH).map(([lx, ly]) =>
    map.containerPointToLatLng(localToScreen(lx, ly, cp.x, cp.y, angle))
  )
...
function clearAll() {
  localRect = null; localCorners = []; lastEmittedBounds = null
  updateHandlePositions()
}
...
function startCornerDrag(el: HTMLElement, i: number, startEvent: PointerEvent) {
  ...
  startCapture(el, startEvent.pointerId, (ev) => {
    ...
    if (Math.abs(lx) / ratio >= Math.abs(ly)) {
      halfW = Math.abs(lx) / 2; halfH = halfW / ratio
    } else {
      halfH = Math.abs(ly) / 2; halfW = halfH * ratio
    }
  ...
  }, () => { map.dragging.enable(); emitBounds() })
}
...
function onBodyDown(startEvent: PointerEvent) {
  ...
  startCapture(rectEl!, startEvent.pointerId, (ev) => {
    if (ev.pointerType === 'touch') ev.preventDefault()
...
watch(() => [props.gridCols, props.gridRows], () => updateGridLines())
...
function setHandlePassthrough(pass: boolean) {
  ...
  cornerEls.forEach(el => { el.style.pointerEvents = pe })
  if (rotateEl) rotateEl.style.pointerEvents = pe
}

function onAltDown(e: KeyboardEvent) { if (e.key === 'Alt') setHandlePassthrough(true) }
function onAltUp(e: KeyboardEvent) { if (e.key === 'Alt') setHandlePassthrough(false) }
