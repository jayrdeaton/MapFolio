...
import { Check, EyeOff, MapPin, Maximize2, Minimize2, Printer, Route, Settings, Type } from '@lucide/vue'
    ...
    <Transition name="mf-focus-right">
      <div v-if="!focusMode" class="flex flex-col gap-2">
        ...
        <button :class="`${fabBase} ${activeFab === 'pins' ? fabActive : fabDefault} relative`" title="Pins" @click="emit('toggle', 'pins')">
          ...
          <span v-else-if="pinCount > 0" class="absolute -top-1 -right-1 min-w-[1.1rem] h-[1.1rem] bg-cyan-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-0.5 ring-2 ring-white dark:ring-zinc-950 pointer-events-none">{{ pinCount > 99 ? '99+' : pinCount }}</span>
        </button>
        <button :class="`${fabBase} ${activeFab === 'routes' ? fabActive : fabDefault} relative`" title="Routes" @click="emit('toggle', 'routes')">
          ...
          <span v-else-if="routeCount > 0" class="absolute -top-1 -right-1 min-w-[1.1rem] h-[1.1rem] bg-cyan-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-0.5 ring-2 ring-white dark:ring-zinc-950 pointer-events-none">{{ routeCount > 99 ? '99+' : routeCount }}</span>
        </button>
        <button :class="`${fabBase} ${activeFab === 'captions' ? fabActive : fabDefault} relative`" title="Captions" @click="emit('toggle', 'captions')">
          ...
          <span v-else-if="captionCount > 0" class="absolute -top-1 -right-1 min-w-[1.1rem] h-[1.1rem] bg-cyan-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-0.5 ring-2 ring-white dark:ring-zinc-950 pointer-events-none">{{ captionCount > 99 ? '99+' : captionCount }}</span>
        </button>
        <button :class="`${fabBase} ${activeFab === 'export' ? fabActive : fabDefault} relative`" title="Print" @click="emit('toggle', 'export')">
          <Printer :size="18" />
          <span v-if="hasPrintArea && activeFab !== 'export'" class="absolute -top-1 -right-1 w-[1.1rem] h-[1.1rem] bg-cyan-500 text-white rounded-full flex items-center justify-center ring-2 ring-white dark:ring-zinc-950 pointer-events-none">
            <Check :size="8" />
          </span>
        </button>
      </div>
