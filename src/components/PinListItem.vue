...
import { Eye, EyeOff, Pencil, X } from '@lucide/vue'

import { Pin } from '../types'

const props = defineProps<{ pin: Pin; hidden?: boolean; resolving?: boolean }>()
...
<template>
  <div :class="['group flex items-center gap-2 py-1.5 rounded hover:bg-gray-50 dark:hover:bg-zinc-800/60 transition-all', hidden ? 'opacity-40' : '']">
    <button
      class="text-base leading-none shrink-0 cursor-pointer hover:scale-125 transition-transform"
      :title="pin.name ? `Zoom to ${pin.name}` : 'Zoom to pin'"
      @click="emit('zoom', pin)"
    >{{ pin.emoji }}</button>
    <div
    ...
    <div class="flex gap-1 opacity-60 hover:opacity-100 transition-opacity">
      <button
        ...
        :title="hidden ? 'Show this pin on map' : 'Hide this pin from map'"
        @click="emit('toggle-visibility', pin.id)"
      >
        <EyeOff v-if="hidden" :size="13" />
        <Eye v-else :size="13" />
      </button>
      <button
        class="w-7 h-7 flex items-center justify-center rounded border border-gray-300 dark:border-zinc-700 text-gray-400 dark:text-zinc-500 hover:bg-gray-100 dark:hover:bg-zinc-700 hover:text-cyan-500 dark:hover:text-cyan-400 cursor-pointer transition-colors"
        title="Edit pin"
        @click="emit('edit', pin)"
      >
        <Pencil :size="13" />
      ...
      <button
        class="w-7 h-7 flex items-center justify-center rounded border border-gray-300 dark:border-zinc-700 text-gray-400 dark:text-zinc-500 hover:border-red-300 dark:hover:border-red-800 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 cursor-pointer transition-colors"
        title="Remove pin"
