...
<template>
  <div
    v-if="show"
    class="fixed bottom-0 left-0 right-0 z-1800 bg-white dark:bg-zinc-900 rounded-t-2xl shadow-2xl no-print max-h-[90vh] overflow-y-auto sm:bottom-6 sm:left-1/2 sm:-translate-x-1/2 sm:right-auto sm:w-110 sm:rounded-2xl"
  >
    <div class="p-4 pb-8">
      ...
      <div class="flex items-center gap-3 mb-4">
        <div class="flex flex-col items-center shrink-0 w-8">
          ...
          <div
            v-if="pinDotSize !== 'none'"
            class="rounded-full border-2 border-white shadow mt-1"
            :style="{ background: newPinColor, width: ({ s: '8px', m: '12px', l: '16px' } as const)[pinDotSize], height: ({ s: '8px', m: '12px', l: '16px' } as const)[pinDotSize] }"
          />
        </div>
        <div class="flex-1 min-w-0">
          <h2 class="text-gray-900 dark:text-zinc-100 text-lg font-bold leading-tight">Edit Pin</h2>
          <p class="text-xs font-mono text-gray-400 dark:text-zinc-500 tabular-nums">
            {{ editingPin?.lat.toFixed(5) }}, {{ editingPin?.lng.toFixed(5) }}
          </p>
      ...
      <div class="space-y-3">
        <div>
          ...
          <input
            type="text"
            v-model="newPinName"
            :placeholder="emojiToName(activeEmoji) || 'Name this pin…'"
            :class="inputClass"
            @keydown.enter="save"
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder='"Great for swimming in summer"'
            :class="inputClass"
            @keydown.enter="save"
          />
        </div>
          ...
          <div class="flex flex-wrap gap-0.5 mb-2">
            <button
              ...
              :class="[
                ...
                newPinEmoji === emoji && !customEmoji.trim()
            ...
            >{{ emoji }}</button>
            <button
              class="w-8 h-8 text-xs rounded cursor-pointer flex items-center justify-center font-medium text-gray-400 dark:text-zinc-500 hover:bg-gray-100 dark:hover:bg-zinc-800"
              @click="showAllEmojis = !showAllEmojis"
            >{{ showAllEmojis ? '−' : '···' }}</button>
          </div>
          <input
            type="text"
            v-model="customEmoji"
        ...
        <div class="flex items-center gap-3">
          <label :class="sectionLabelClass" style="margin-bottom:0">Color</label>
          <input
            type="color"
            v-model="newPinColor"
            class="w-10 h-9 p-0.5 cursor-pointer rounded border border-gray-300 dark:border-zinc-700"
          />
        </div>
        ...
        <div class="flex gap-2">
          <button
            ...
            @click="emit('delete')"
          >
            <Trash2 :size="15" /> {{ pendingPin?.id === editingPin?.id ? 'Cancel' : 'Delete' }}
          </button>
          <button
