...
<template>
  <div class="p-4 overflow-y-auto space-y-4">
    <!-- Map title -->
    <div>
      <label for="mapName" :class="sectionLabelClass">Map Title</label>
      <input id="mapName" v-model="mapName" type="text" placeholder="My Map" :class="inputClass" />
    </div>

    <!-- Area -->
    <div>
      <label for="mapArea" :class="sectionLabelClass">Area</label>
      <input id="mapArea" v-model="mapArea" type="text" :placeholder="isAutoArea ? 'Loading…' : 'e.g. Yellowstone, Wyoming'" :class="inputClass" />
    </div>
    ...
    <div class="pt-3 border-t border-gray-100 dark:border-zinc-800">
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex gap-1">
          <button class="w-7 h-7 flex items-center justify-center rounded border border-cyan-400 bg-cyan-500 text-white hover:bg-cyan-600 cursor-pointer text-base leading-none" title="Set print area" @click="printPaper && printOrientation && emit('select-preset', printPaper, printOrientation)">+</button>
          <button class="w-7 h-7 flex items-center justify-center rounded border border-gray-300 dark:border-zinc-700 text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 cursor-pointer transition-colors" title="Fit print area to visible pins" @click="emit('fit-to-pins')"><ScanSearch :size="13" /></button>
          <button class="w-7 h-7 flex items-center justify-center rounded border border-gray-300 dark:border-zinc-700 text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 cursor-pointer text-base leading-none disabled:opacity-40 disabled:cursor-not-allowed" title="Clear print area" :disabled="!printBounds" @click="emit('clear-print-bounds')">−</button>
          <button :class="`w-7 h-7 flex items-center justify-center rounded border cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${printSnapEnabled ? 'border-cyan-400 bg-cyan-500 text-white hover:bg-cyan-600' : 'border-gray-300 dark:border-zinc-700 text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800'}`" title="Toggle rotation snap" :disabled="!printBounds" @click="printSnapEnabled = !printSnapEnabled"><Magnet :size="13" /></button>
          <button class="w-7 h-7 flex items-center justify-center rounded border border-gray-300 dark:border-zinc-700 text-gray-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed" title="Fit map to print area" :disabled="!printBounds" @click="emit('fit-to-print-area')"><Maximize2 :size="13" /></button>
        </div>
      </div>
    ...
    <div class="pt-3 border-t border-gray-100 dark:border-zinc-800">
      ...
      <div class="space-y-1.5">
        <div class="flex items-center justify-between">
          ...
          <button :class="`h-6 px-2.5 rounded text-xs font-medium border cursor-pointer transition-colors ${includeLegend ? 'border-cyan-400 bg-cyan-500 text-white hover:bg-cyan-600' : 'border-gray-300 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800'}`" @click="includeLegend = !includeLegend">{{ includeLegend ? 'On' : 'Off' }}</button>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-gray-600 dark:text-zinc-400">Compass</span>
          <button :class="`h-6 px-2.5 rounded text-xs font-medium border cursor-pointer transition-colors ${includeCompass ? 'border-cyan-400 bg-cyan-500 text-white hover:bg-cyan-600' : 'border-gray-300 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800'}`" @click="includeCompass = !includeCompass">{{ includeCompass ? 'On' : 'Off' }}</button>
        </div>
        <div class="flex items-center justify-between">
          <span class="text-xs text-gray-600 dark:text-zinc-400">Scale</span>
          <div class="flex gap-1">
            <button v-for="opt in ['off', 'km', 'mi'] as const" :key="opt" :class="`h-6 px-2.5 rounded text-xs font-medium border cursor-pointer transition-colors ${scaleUnit === opt ? 'border-cyan-400 bg-cyan-500 text-white hover:bg-cyan-600' : 'border-gray-300 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800'}`" @click="scaleUnit = opt">{{ opt === 'off' ? 'Off' : opt }}</button>
          </div>
        </div>
        <div v-if="MAP_STYLE_CONFIGS[mapStyle].printBlackPoint !== undefined" class="flex items-center justify-between">
          ...
          <button :class="`h-6 px-2.5 rounded text-xs font-medium border cursor-pointer transition-colors ${enhanceContrast ? 'border-cyan-400 bg-cyan-500 text-white hover:bg-cyan-600' : 'border-gray-300 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800'}`" @click="enhanceContrast = !enhanceContrast">{{ enhanceContrast ? 'On' : 'Off' }}</button>
        </div>
      </div>
    </div>
    ...
    <!-- Download -->
    <div class="pt-3 border-t border-gray-100 dark:border-zinc-800">
      <button :class="`${btnBase} w-full justify-center bg-cyan-500 text-white hover:bg-cyan-600 disabled:opacity-50 disabled:cursor-not-allowed`" :disabled="isDownloadingPdf || !printBounds" @click="emit('download-pdf')"><Printer :size="14" /> {{ isDownloadingPdf ? 'Building…' : 'PDF' }}</button>
