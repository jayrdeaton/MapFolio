...
import type { PrintPaperSize, PrintOrientation } from './usePrintSettings'
...
export function usePrintExport(options: {
  ...
  printSettings: {
    ...
    autoTitle: Ref<boolean>
  }
}) {
  const { leafletMap, mapStyle, mapTitle, pins, hiddenPinIds, showNotification, printSettings } = options
  ...
  watch(printPaper, v => { if (v) printSettings.paper.value = v })
  watch(printOrientation, v => { if (v) printSettings.orientation.value = v })
  ...
  async function fetchTitleForBounds(bounds: L.LatLngBounds) {
    const { lat, lng } = bounds.getCenter()
    isAutoTitling.value = true
    try {
      ...
      mapTitle.value = city && region ? `${city}, ${region}` : city || region || data.display_name || ''
    } catch { } finally {
      isAutoTitling.value = false
    }
  }

  let autoTitleTimer: ReturnType<typeof setTimeout> | null = null

  watch(printBounds, (bounds) => {
    if (!printSettings.autoTitle.value || !bounds) return
    if (autoTitleTimer) clearTimeout(autoTitleTimer)
    autoTitleTimer = setTimeout(() => fetchTitleForBounds(bounds), 800)
  })

  watch(printSettings.autoTitle, (on) => {
    if (on && printBounds.value) fetchTitleForBounds(printBounds.value)
  })
  ...
  function calculatePrintBounds(widthToHeight: number): L.LatLngBounds {
    ...
    return L.latLngBounds(
      map.containerPointToLatLng(L.point(cx - pw / 2, cy + ph / 2)),
      map.containerPointToLatLng(L.point(cx + pw / 2, cy - ph / 2))
    )
  }
  ...
  function resnapPrintArea() {
    ...
    printBounds.value = calculatePrintBounds(
      printAspectRatio.value ?? ASPECT_RATIOS[printPaper.value][printOrientation.value]
    )
  }
  ...
  function fitToPrintArea() {
    ...
    leafletMap.value.fitBounds([[sw.lat, sw.lng], [ne.lat, ne.lng]], { padding: [40, 40], animate: true })
  }

  function fitPrintAreaToPins() {
    if (!leafletMap.value) return
    const visible = pins.value.filter(p => !hiddenPinIds.value.has(p.id))
    if (visible.length === 0) { showNotification('No visible pins to fit', 'error'); return }
    ...
    if (maxX - minX < PAD * 2) { const mx = (minX + maxX) / 2; minX = mx - PAD; maxX = mx + PAD }
    if (maxY - minY < PAD * 2) { const my = (minY + maxY) / 2; minY = my - PAD; maxY = my + PAD }
    ...
    map.fitBounds([[sw.lat, sw.lng], [ne.lat, ne.lng]], { padding: [40, 40], animate: true })
  }

  function computeOverlayCorner(): 0 | 1 | 2 | 3 {
    if (printCorners.value.length !== 4) return 2
    const [nw, ne, se, sw] = printCorners.value
    ...
    let min = Infinity
    for (let i = 0; i < 4; i++) {
      if (counts[i] < min) { min = counts[i]; best = i as 0 | 1 | 2 | 3 }
    }
    return best
  }
  ...
  async function downloadPdf() {
    ...
    try {
      ...
      a.href = url
      a.download = `${mapTitle.value || 'mapfolio'}.pdf`
      a.click()
  ...
  return {
    printBounds, printCorners, printAngle, printSnapEnabled, isDownloadingPdf, isAutoTitling,
    printPaper, printOrientation, parsedGrid, printAspectRatio, overlayCorner,
