...
export function useMapClipboard() {
  ...
  const hasClipboard = computed(() => !!clipboard.value && clipboard.value.pins.length + clipboard.value.routes.length + clipboard.value.captions.length > 0)
  const sourceMapId = computed(() => clipboard.value?.sourceMapId ?? null)

  function set(pins: Pin[], routes: Route[], captions: Caption[] = [], mapId?: string) {
    clipboard.value = {
      pins: pins.map((p) => ({ ...p })),
  ...
  function pasteAt(targetLat: number, targetLng: number): Omit<MapClipboard, 'sourceMapId'> | null {
    ...
    const { pins, routes, captions } = clipboard.value

    const allLats = [...pins.map((p) => p.lat), ...routes.flatMap((r) => r.points.map((pt) => pt.lat)), ...captions.map((c) => c.lat)]
    const allLngs = [...pins.map((p) => p.lng), ...routes.flatMap((r) => r.points.map((pt) => pt.lng)), ...captions.map((c) => c.lng)]
    ...
    const newCaptions: Caption[] = captions.map((c) => ({
      ...c,
      id: uid(),
      lat: c.lat + dLat,
      lng: c.lng + dLng
    ...
    return { pins: newPins, routes: newRoutes, captions: newCaptions }
  }
